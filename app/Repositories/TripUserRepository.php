...
class TripUserRepository extends BaseRepository
{
    ...
    public function inviteFriends($tripId, $friendIds)
    {
        for ($i = 0; $i < count($friendIds); $i++) {
            if (! $this->findWhere(['trip_id' => $tripId, 'user_id' => $friendIds[$i]['id']])->first()) {
                $member = $this->firstOrCreate([
                    'trip_id' => $tripId,
                    'user_id' => $friendIds[$i]['id'],
    ...
    public function deleteOldFriends($tripId, $userId) {
       $userTrips = $this->findWhere(['trip_id' => $tripId]);
       foreach ($userTrips as $userTrip) {
        $this->delete($userTrip->id);
       }
    }
    ...
    {
        return $this->with('trip')->findWhere(['user_id' => $userId, 'type' => self::INVITATION, 'status' => self::PENDING]);
    }
