...
class FriendRepository extends BaseRepository
{
    ...
    {
        $friendship = $this->with(['userOne', 'userTwo'])->findWhere(['user_one_id' => $userOne, 'user_two_id' => $userTwo])->first();
        if ($friendship) {
            return $friendship;
        } else {
            $friendship = $this->with(['userOne', 'userTwo'])->findWhere(['user_one_id' => $userTwo, 'user_two_id' => $userOne])->first();
            return $friendship;
        }
        return $friendship;
    }
